...
#                             0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10])
  ...
  def LoadBMP(self, fn):
      ...
      if f.read(2) == b'BM':  #header
          ...
          hdrsize = int.from_bytes(f.read(4), endian)
          width = int.from_bytes(f.read(4), endian)
          height = int.from_bytes(f.read(4), endian)
          if int.from_bytes(f.read(2), endian) == 1: #planes must be 1
              depth = int.from_bytes(f.read(2), endian)
              if depth == 24 and int.from_bytes(f.read(4), endian) == 0:#compress method == uncompressed
