...
ADS1115_RNG = (ADS1115_RANGE_0256, ADS1115_RANGE_0512, ADS1115_RANGE_1024, 
    ADS1115_RANGE_2048, ADS1115_RANGE_4096, ADS1115_RANGE_6144)
...
ADS1115_CH = (ADS1115_COMP_0_GND, ADS1115_COMP_1_GND, ADS1115_COMP_2_GND, ADS1115_COMP_3_GND,
...
class ADS1115(object):
    ...
    def __init__(self, address = 0x48, i2c = None, i2c_id = 0, sda=Pin(0), scl=Pin(1)):
        self.__address = address
        if i2c is None: i2c = I2C(i2c_id, scl=scl, sda=sda, freq=100000, timeout=5000)
        # print(i2c)
        self.__i2c = i2c
        try:
            self.reset()
    ...
    def readMulti(self, ChList):
        result = [] 
        for ChN in ChList:
            self.setCompareChannels(ADS1115_CH[ChN])
            t = 0
            for i in range(self._AVG_CNT):
                self.startSingleMeasurement()
                while(self.isBusy()): sleep_us(100)
                t += self.getResult_V()
            avg = round(t/self._AVG_CNT,5)
            result.append(avg)
        return tuple(result)
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        while(adc.isBusy()): pass
        print(adc.getResult_V())